...
> **Deep dive**: For production async patterns such as stream processing, graceful shutdown, connection pooling, and cancellation safety, see the companion [Async Rust Training](../async-book/) guide.<br><span class="zh-inline">**深入阅读：** 如果要继续看生产环境里的异步模式，例如流处理、优雅停机、连接池、取消安全，可以接着读配套的 [Async Rust Training](../async-book/) 指南。</span>
>
