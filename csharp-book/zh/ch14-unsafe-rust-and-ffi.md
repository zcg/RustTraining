...
> **Advanced coverage**: For safe abstraction patterns over unsafe code, such as arena allocators, lock-free structures, and custom vtables, see [Rust Patterns](../../source-docs/RUST_PATTERNS.md).<br><span class="zh-inline">**更深入的延伸阅读：** 如果想继续看如何在 unsafe 之上建立安全抽象，例如 arena 分配器、无锁结构和自定义 vtable，可以去读 [Rust Patterns](../../source-docs/RUST_PATTERNS.md)。</span>
