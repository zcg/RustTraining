...
In addition to `cargo-expand`, you can also use rust-analyzer to expand macros:<br><span class="zh-inline">除了 `cargo-expand`，也可以直接借助 rust-analyzer 在编辑器里展开宏：</span>

1. Move cursor to the macro you want to check.<br><span class="zh-inline">1. 把光标放到想查看的宏上。</span>
2. Open command palette (e.g. `F1` on VSCode).<br><span class="zh-inline">2. 打开命令面板，例如 VSCode 里的 `F1`。</span>
3. Search for `rust-analyzer: Expand macro recursively at caret`.<br><span class="zh-inline">3. 搜索 `rust-analyzer: Expand macro recursively at caret` 并执行。</span>
